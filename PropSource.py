  ...
  Subset random: It randomly chooses, without repetition, the samples from each subset.
	...
	##repeat the simulation n times varying nY, nCB, nUR and nCF
        ...
        for ss in samples_sizes:
    	    ...
    	    if ss in plots2D:
    	        self.plot2D(P_0, ss, n, idx, mean=True, convex_hull = True, title = "Confidence region 95%_OLS_Clarke Model")
    	        self.plot2D(P_2, ss, n, idx, mean=True, convex_hull = True, title = "Confidence region 95%_GLS_Clarke Model")
         ###### #####
